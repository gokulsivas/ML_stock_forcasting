...
import { TrendingUp, Plus, X, BarChart3, Trash2, Download, FileSpreadsheet, AlertCircle } from 'lucide-react';
...
function Comparison() {
  ...
  const [stockError, setStockError] = useState(null);       // ✅ for fetch stocks error only
  const [predictionError, setPredictionError] = useState(null); // ✅ for compare errors only
  ...
  useEffect(() => {
    ...
    const savedDays = localStorage.getItem('comparisonDays');
    if (savedStocks) {
      try {
        const parsed = JSON.parse(savedStocks);
        const valid = parsed.filter(s => s && typeof s.value === 'string');
        setSelectedStocks(valid);
      } catch (e) {
        console.error('Failed to load saved stocks');
      }
    }
    if (savedDays) setDaysAhead(parseInt(savedDays));
  }, []);


  useEffect(() => {
    if (selectedStocks.length > 0) {
      localStorage.setItem('comparisonStocks', JSON.stringify(selectedStocks));
    } else {
  ...
  useEffect(() => {
    localStorage.setItem('comparisonDays', daysAhead);
  }, [daysAhead]);
  ...
  const compareStocks = async () => {
    if (selectedStocks.length === 0) {
      setPredictionError('Please select at least one stock');
      return;
    }
    ...
    for (const stock of selectedStocks) {
      try {
        ...
        failedStocks.push(stock.value);
      }
    }

    if (newPredictions.length === 0) {
      setPredictionError('All predictions failed. These stocks may not have enough data.');
    } else {
      setPredictions(newPredictions);
      if (failedStocks.length > 0) {
        setPredictionError(`Skipped: ${failedStocks.join(', ')} — insufficient data`);
      }
    }
  ...
  const exportComparison = (format) => {
    ...
    if (format === 'csv') {
      ...
      predictions.forEach(pred => {
        ...
        csvData.push([pred.symbol, pred.current_price.toFixed(2), lastPred.predicted_price.toFixed(2), change, recommendation]);
      });
      ...
      predictions.forEach(pred => {
        ...
        const change = ((lastPred.predicted_price - pred.current_price) / pred.current_price * 100).toFixed(2);
        const recommendation = change > 5 ? 'Strong Buy' : change > 2 ? 'Buy' : change > -2 ? 'Hold' : change > -5 ? 'Sell' : 'Strong Sell';
        summaryData.push([pred.symbol, pred.current_price, lastPred.predicted_price, parseFloat(change), recommendation]);
      });
      ...
      predictions.forEach(pred => {
        const stockData = [['Date', 'Predicted Price', 'Daily Return (%)']];
  ...
  const selectStyles = {
    ...
    menuList: (base) => ({
      ...
      '::-webkit-scrollbar-thumb': { background: theme === 'light' ? '#cbd5e1' : '#475569', borderRadius: '4px' }
    }),
    option: (base, state) => ({
      ...base,
    ...
    singleValue: (base) => ({ ...base, color: theme === 'light' ? '#0f172a' : '#e2e8f0', fontWeight: '500' }),
    input: (base) => ({ ...base, color: theme === 'light' ? '#0f172a' : '#e2e8f0' }),
  ...
  return (
    <div className="p-8 animate-[fadeIn_0.5s_ease-in]">
      ...
      <div className={`w-full ${
            ...
            <div className="flex gap-2">
              <div className="flex-1">
                <Select
                  ...
                  options={stocks.filter(s => !selectedStocks.find(sel => sel.value === s.value))}
                  isDisabled={loading}
                  isSearchable={true}
        ...
        {/* ✅ Stock fetch error — only shows if /api/stocks/ fails */}
        {stockError && (
        ...
        {/* ✅ Prediction error — only shows after clicking Compare */}
        {predictionError && (
          <div className="bg-red-500/10 border border-red-500/30 text-red-400 px-5 py-4 rounded-xl mb-6 font-medium flex items-center gap-3">
            <AlertCircle size={20} />
            {predictionError}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className={`text-xs ${theme === 'light' ? 'text-slate-600' : 'text-slate-400'} uppercase font-semibold tracking-wider mb-1`}>Current Price</div>
                        <div className={`text-2xl font-bold ${theme === 'light' ? 'text-slate-900' : 'text-slate-200'}`}>₹{pred.current_price.toFixed(2)}</div>
                      </div>
                      <div>
                        <div className={`text-xs ${theme === 'light' ? 'text-slate-600' : 'text-slate-400'} uppercase font-semibold tracking-wider mb-1`}>Predicted Price</div>
                        <div className={`text-2xl font-bold ${theme === 'light' ? 'text-slate-900' : 'text-slate-200'}`}>₹{lastPred.predicted_price.toFixed(2)}</div>
                      </div>
                      <div>
                        <div className={`text-xs ${theme === 'light' ? 'text-slate-600' : 'text-slate-400'} uppercase font-semibold tracking-wider mb-1`}>Expected Change</div>
                      ...
                      <div className={`pt-3 border-t ${theme === 'light' ? 'border-slate-200' : 'border-white/10'}`}>
                        <div className={`text-xs ${theme === 'light' ? 'text-slate-600' : 'text-slate-400'} uppercase font-semibold tracking-wider mb-1`}>Recommendation</div>
