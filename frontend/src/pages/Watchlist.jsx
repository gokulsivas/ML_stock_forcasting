...
function Watchlist() {
  ...
  const [failedStocks, setFailedStocks] = useState({});  // ← NEW: track per-stock failures
  ...
  const fetchAllPredictions = async () => {
    ...
    // ── Promise.allSettled: each stock succeeds/fails independently ──────────
    // Promise.all (old) would crash everything if one stock failed
    ...
    results.forEach((result, idx) => {
      ...
      if (result.status === 'fulfilled') {
        ...
        const detail = result.reason?.response?.data?.detail || 'Insufficient data';
        failedMap[symbol] = detail;
      }
    });
        ...
        {watchlist.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {watchlist.map((stock) => {
              ...
              const failReason   = failedStocks[stock.value];  // ← NEW
              const hasPrediction = !!prediction;
              const hasFailed     = !!failReason;              // ← NEW
                  ...
                  {loadingPredictions && !hasPrediction && !hasFailed ? (
                    ...
                    <div className={`py-6 text-center flex flex-col items-center gap-3`}>
                      ...
                      <p className={`text-xs ${
                        ...
                        Not enough historical data to predict
                      </p>
                    </div>
